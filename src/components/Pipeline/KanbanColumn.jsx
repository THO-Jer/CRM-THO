...
export default function KanbanColumn({estado, prospectos, onEdit, onDelete, onMove, onCerrar, getEstadoFromKey, onHistory, onConvert, onDetail}) {
    ...
    return (
        <div className="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-4 kanban-column min-h-[200px]" onDragOver={handleDragOver} onDrop={handleDrop}>
            <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-gray-800 dark:text-gray-200">{estado.emoji} {estado.nombre}</h3>
                <span className="text-xs bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 font-medium px-2 py-1 rounded-full shadow-sm">{prospectos.length}</span>
            </div>
