...
const HTML_ENTITIES: Record<string, string> = {
    '&deg;': '°', '&oacute;': 'ó', '&aacute;': 'á', '&eacute;': 'é',
    '&iacute;': 'í', '&uacute;': 'ú', '&ntilde;': 'ñ', '&Oacute;': 'Ó',
...
/**
 * Lee un archivo (File del input) y devuelve el workbook.
 */
...
export interface BoletaRow {
    ...
    rut: string | null
    rut_prestador: string | null
    numero_boleta: string | null
    estado: string | null
    fecha_anulacion: string | null
    sociedad_profesional: boolean | null
    monto_bruto_clp: number
    monto_bruto_uf: number
    monto_retencion_clp: number | null
    monto_retenido_clp: number | null
    monto_liquido_clp: number | null
    monto_pagado_clp: number | null
    uf_dia: number
...
export interface FacturaEmitidaRow {
    ...
    acteco_emisor: string | null
    codigo_sii_sucursal: string | null
    direccion_emisor: string | null
    comuna_emisor: string | null
    ciudad_emisor: string | null
    ...
    total_neto_clp: number | null
    total_exento_clp: number | null
    total_iva_clp: number | null
    total_monto_clp: number | null
    monto_clp: number | null
    monto_uf: number | null
    uf_dia: number | null
    estado: 'Pendiente'
    monto_periodo_clp: number | null
    monto_no_facturable_clp: number | null
    saldo_anterior_clp: number | null
    valor_pagar_clp: number | null
    detalle_descripcion?: string | null
    detalle_cantidad?: number | null
    detalle_precio_clp?: number | null
    detalle_monto_item_clp?: number | null
    origen: 'sii_xls'
    fuente: 'sii_xls'
    nombre_archivo_origen: string
    periodo_anio: number
...
export interface FacturaRecibidaRow {
    numero_factura: string
...
export function parseBoletasHonorarios(
    ...
    if (headerIdx === -1) throw new Error('No se encontró la fila de encabezados. ¿Es el Excel correcto?')
    ...
    for (let i = headerIdx + 1; i < rows.length; i++) {
        ...
        result.push({
            fecha, fecha_emision: fecha, prestador: nombre, rut, rut_prestador: rut,
            numero_boleta: toText(r[idx.numero]), estado: toText(r[idx.estado]),
            ...
            uf_dia: ufActual || 0, moneda_principal: 'CLP',
            periodo_anio: parseInt(periodo[0], 10),
            periodo_mes: parseInt(periodo[1], 10),
            origen: 'sii_xls', fuente: 'sii_xls', nombre_archivo_origen: fileName,
        })
    }
    return result
}
...
function parseFacturasGenerico(
    ...
    const findAll = (name: string) => headers.map((h, i) => (h === name ? i : -1)).filter((i) => i !== -1)
    const findFirst = (name: string) => headers.indexOf(name)
    ...
    const idx = {
        tipoDTE: findFirst('TipoDTE'), folio: findFirst('Folio'),
        fechaEmision: findFirst('FechaEmision'), tipoDespacho: findFirst('TipoDespacho'),
        formaPago: findFirst('FormaPago'), rutEmisor: findFirst('RutEmisor'),
        razonSocialEmisor: findFirst('RazonSocialEmisor'), giroEmisor: findFirst('GiroEmisor'),
        acteco: findFirst('Acteco'), codSII: findFirst('CodSIISucursal'),
        direccionEmisor: directionIdxs[0], comunaEmisor: comunaIdxs[0], ciudadEmisor: ciudadIdxs[0],
        rutReceptor: findFirst('RutReceptor'), razonSocialReceptor: findFirst('RazonSocialReceptor'),
        giroReceptor: findFirst('GiroReceptor'),
        direccionReceptor: directionIdxs[1], comunaReceptor: comunaIdxs[1], ciudadReceptor: ciudadIdxs[1],
        totalNeto: findFirst('Total-Neto'), totalExento: findFirst('Total-Exento'),
        totalIVA: findFirst('Total-IVA'), totalMonto: findFirst('Total-MontoTotal'),
        montoPeriodo: findFirst('MontoPeriodo'), montoNoFacturable: findFirst('Monto-NoFacturable'),
        saldoAnterior: findFirst('Saldo-Anterior'), valorPagar: findFirst('ValorPagar'),
    }

    if (idx.folio === -1 || idx.fechaEmision === -1 || idx.razonSocialReceptor === -1 || idx.razonSocialEmisor === -1) {
        throw new Error('Faltan columnas obligatorias del SII. ¿Es el Excel correcto?')
    }

    const result: (FacturaEmitidaRow & { __hasItem?: boolean } | FacturaRecibidaRow & { __hasItem?: boolean })[] = []
    let current: (FacturaEmitidaRow & { __hasItem?: boolean } | FacturaRecibidaRow & { __hasItem?: boolean }) | null = null
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (c0 === '' && String(r[1] || '').trim() !== '') {
            if (current && !(current as FacturaEmitidaRow & { __hasItem?: boolean }).__hasItem && mode === 'emitidas') {
                const fe = current as FacturaEmitidaRow & { __hasItem?: boolean }
                fe.detalle_descripcion = toText(r[4])
                fe.detalle_cantidad = parseNumber(r[5])
                fe.detalle_precio_clp = parseNumber(r[6])
                fe.detalle_monto_item_clp = parseNumber(r[10])
                fe.__hasItem = true
            }
            continue
        }
        ...
        if (mode === 'emitidas') {
            current = {
                tipo_dte: tipoDTE, tipo_documento: tipoDTE, folio,
                numero_folio: parseInt(folio, 10) || null, numero_factura: folio,
                fecha_emision: fechaEmision,
                tipo_despacho: toText(r[idx.tipoDespacho]), forma_pago: toText(r[idx.formaPago]),
                rut_emisor: toText(r[idx.rutEmisor]), razon_social_emisor: toText(r[idx.razonSocialEmisor]),
                giro_emisor: toText(r[idx.giroEmisor]), acteco_emisor: toText(r[idx.acteco]),
                codigo_sii_sucursal: toText(r[idx.codSII]),
                direccion_emisor: toText(r[idx.direccionEmisor]), comuna_emisor: toText(r[idx.comunaEmisor]),
                ciudad_emisor: toText(r[idx.ciudadEmisor]), rut_receptor: toText(r[idx.rutReceptor]),
                rut_cliente: toText(r[idx.rutReceptor]), razon_social_receptor: toText(r[idx.razonSocialReceptor]),
                cliente: toText(r[idx.razonSocialReceptor]), giro_receptor: toText(r[idx.giroReceptor]),
                direccion_receptor: toText(r[idx.direccionReceptor]), comuna_receptor: toText(r[idx.comunaReceptor]),
                ciudad_receptor: toText(r[idx.ciudadReceptor]),
                total_neto_clp: parseNumber(r[idx.totalNeto]), total_exento_clp: parseNumber(r[idx.totalExento]),
                total_iva_clp: parseNumber(r[idx.totalIVA]), total_monto_clp: totalMonto,
                monto_clp: totalMonto, monto_uf: montoUF, uf_dia: ufActual || null, estado: 'Pendiente',
                monto_periodo_clp: parseNumber(r[idx.montoPeriodo]),
                monto_no_facturable_clp: parseNumber(r[idx.montoNoFacturable]),
                saldo_anterior_clp: parseNumber(r[idx.saldoAnterior]), valor_pagar_clp: parseNumber(r[idx.valorPagar]),
                origen: 'sii_xls', fuente: 'sii_xls', nombre_archivo_origen: fileName ?? '',
                periodo_anio: parseInt(fechaEmision.split('-')[0], 10),
                periodo_mes: parseInt(fechaEmision.split('-')[1], 10),
                __hasItem: false,
            } as FacturaEmitidaRow & { __hasItem: boolean }
        } else {
            current = {
                numero_factura: folio, proveedor: toText(r[idx.razonSocialEmisor]),
                fecha_emision: fechaEmision, monto_clp: totalMonto,
                monto_uf: montoUF, uf_dia: ufActual || null,
                categoria: 'Otros', estado: 'Pendiente',
                descripcion: toText(r[idx.giroEmisor]) || null,
                __hasItem: false,
            } as FacturaRecibidaRow & { __hasItem: boolean }
        }
    }
    if (current) {
        delete (current as unknown as Record<string, unknown>).__hasItem
