...
const servicioDefaults: Record<string, ServiceDefault> = {
...
export default function ProposalGenerator({ prospecto, onClose, ufActual }: ProposalGeneratorProps) {
    ...
    const generatePDF = () => {
        ...
        type TextOptions = { size?: number; style?: 'normal' | 'bold' | 'italic'; color?: [number, number, number]; align?: 'left' | 'center' | 'right'; maxWidth?: number }

        const addText = (text: string, x: number, yPos: number, options: TextOptions = {}) => {
            const { size = 10, style = 'normal', color = [51, 51, 51] as [number, number, number], align = 'left', maxWidth = contentW } = options
        ...
        const addLine = (yPos: number, color: [number, number, number] = [230, 230, 230]) => {
        ...
        form.entregables.split('\n').filter(e => e.trim()).forEach(entregable => {
            checkPage(8)
