...
export default function HistoryModal({ open, title, items, loading, onClose }: HistoryModalProps) {
    useEscapeKey(onClose, open)
    if (!open) return null

    const formatDT = (dt: string | undefined) => {
        if (!dt) return ''
        try {
    ...
    const getDetalles = (item: HistoryItem) => {
        if (item.payload) {
            const { old: antes, new: despues } = item.payload as { old?: Record<string, unknown>; new?: Record<string, unknown> }
                                                ...
                                                <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="mt-2 space-y-1">
                                                            {detalles.map((d, i) => (
                                                                <div key={i} className="text-xs bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg">
                                                                    ...
                                                                    <span className="text-gray-400 mx-1">{String(d.antes)}</span>
                                                                    <span className="text-gray-300 dark:text-gray-500">→</span>
                                                                    <span className="text-gray-800 dark:text-gray-200 ml-1 font-medium">{String(d.despues)}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    )}
                                                    {it.kind === 'transition' && it.payload && (
                                                        <div className="text-xs text-gray-600 dark:text-gray-300 mt-2 bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg">
                                                            <span className="font-medium">Transición:</span> {String(it.payload.from)} → {String(it.payload.to)}
                                                            {it.payload.notes && <div className="mt-1 text-gray-500 dark:text-gray-400">{String(it.payload.notes)}</div>}
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
