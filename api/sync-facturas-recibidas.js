...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!certB64 && !(certFileBuffer && certFileBuffer.length > 0)) {
      return res.status(400).json({
        error: 'Falta certificado digital (certificadoB64 o archivo) para RCV compras',
        statusCode: 400,
        urlPath: null,
        mode: mode || 'cert-rcv',
        details: null,
    ...
    const urlPath = `/api/RCV/compras/${mesPad}/${anioNum}`;
    ...
    const boundary = `----crm-tho-rcv-compras-${Date.now().toString(16)}`;
    const chunks = [];
