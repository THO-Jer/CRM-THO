...
// MÓDULO: CONTABILIDAD
// The Human Org - CRM
// Carga dinámica compatible con Babel
// ============================================
...
(function() {
    ...
    function ContabilidadView({ 
        ...
        onFiles 
            ...
            const sueldosSociosFiltrados = React.useMemo(() => {
                const hoy = new Date();
                ...
                const timeout = setTimeout(() => {
                    ...
                    if (canvasIG && window.dashboardData) {
                        ...
                        canvasIG.chart = new Chart(canvasIG, {
                            type: 'bar',
                            data: {
                                labels: window.dashboardData.datos6Meses.map(d => d.label),
                                datasets: [
                                    {
                                        label: 'Ingresos',
                                        data: window.dashboardData.datos6Meses.map(d => d.ingresos),
                                    ...
                                    {
                                        label: 'Gastos',
                                        data: window.dashboardData.datos6Meses.map(d => d.gastos),
                    ...
                    if (canvasD && window.dashboardData) {
                        if (canvasD.chart) {
                            canvasD.chart.destroy();
                            canvasD.chart = null;
                        }
                        const totalG = window.dashboardData.gastosActual + window.dashboardData.honorariosActual + window.dashboardData.cajaActual;
                        canvasD.chart = new Chart(canvasD, {
                            ...
                            data: {
                                ...
                                datasets: [{
                                    data: [
                                        totalG > 0 ? Math.round(window.dashboardData.gastosActual) : 0,
                                        totalG > 0 ? Math.round(window.dashboardData.honorariosActual) : 0,
                                        totalG > 0 ? Math.round(window.dashboardData.cajaActual) : 0
                                    ],
            ...
            return (
                <div className="space-y-6">
                    ...
                    {(() => {
                        ...
                        window.exportarSueldosExcel = exportarSueldosExcel;
                        return null;
                    })()}
                        ...
                        <div className="p-6">
                            ...
                            {contaTab === 'dashboard' && (() => {
                                ...
                                <div className="space-y-6">
                                    ...
                                    {(window.dashboardData = { datos6Meses, gastosActual, honorariosActual, cajaActual }, null)}
                                </div>
                                );
                            })()}
                                                    ...
                                                    <tr key={f.id} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-4 py-3 text-right space-x-2">
                                                            {f.estado !== 'Reclamado' && f.estado !== 'Reclamada' && (
                                                                <button 
                                                                    onClick={async () => {
                                                                        if (confirm(`¿Anular factura #${f.numero_factura}? Esta acción la excluirá de los cálculos.`)) {
                                                                            const result = await supabase
                                                                                .from('facturas_emitidas')
                                                    ...
                                                    <tr key={f.id} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-4 py-3 text-right space-x-2">
                                                            {f.estado !== 'Reclamado' && f.estado !== 'Reclamada' && (
                                                                <button 
                                                                    onClick={async () => {
                                                                        if (confirm(`¿Anular factura de ${f.proveedor}? Esta acción la excluirá de los cálculos.`)) {
                                                                            const result = await supabase
                                                                                .from('facturas_recibidas')
                                                                                .update({ estado: 'Reclamada' })
                                                                                .eq('id', f.id);
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center flex-wrap gap-3">
                                        ...
                                        <div className="flex gap-2 flex-wrap">
                                            ...
                                            <button 
                                                onClick={() => window.exportarSueldosExcel(sueldosSociosFiltrados, filtroSueldosPeriodo)}
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex gap-2">
                                            <button 
                                                onClick={async () => {
                                                    if (confirm('¿Eliminar TODOS los movimientos bancarios? Esta acción no se puede deshacer.')) {
                                                        const { error } = await supabase.from('movimientos_bancarios').delete().neq('id', '00000000-0000-0000-0000-000000000000');
                                                        if (error) alert('Error: ' + error.message);
                                                        else { alert('✅ Movimientos eliminados'); onReload(); }
                                                    }
                                                }}
                                                    ...
                                                    <div key={mov.id} className="bg-white p-4 rounded-lg border">
                                                        ...
                                                        {mejorMatch && mejorMatch.score >= 0.60 ? (
                                                            <div className="bg-blue-50 p-3 rounded mb-2">
                                                                ...
                                                                <button 
                                                                    onClick={() => {
                                                                        if (confirm(`¿Conciliar con ${mejorMatch.descripcion}?`)) {
                                                                        ...
                                                                        <div className="mt-2 space-y-1">
                                                                            {resultado.matches.slice(1, 4).map((match, idx) => (
                                                                                <div key={idx} className="text-xs flex justify-between items-center p-2 bg-white rounded">
                                                                                    ...
                                                                                    <button 
                                                                                        onClick={() => {
                                                                                            if (confirm(`¿Conciliar con ${match.descripcion}?`)) {
                                                            ...
                                                            <div className="bg-yellow-50 p-3 rounded mb-2">
                                                                ...
                                                                <button 
                                                                    onClick={() => {
                                                                        if (confirm(`¿Crear gasto en Caja Chica como "${resultado.sugerenciaCategoria}"?`)) {
                                                        ...
                                                        {mov.tipo === 'entrada' && (!resultado.matches || resultado.matches.length === 0 || resultado.matches[0].score < 0.60) && (
                                                            <div className="bg-green-50 p-3 rounded mb-2">
                                                                ...
                                                                <button 
                                                                    onClick={() => alert('Próximamente: crear factura emitida prellenada')}
                                                        ...
                                                        <div className="flex gap-2 pt-2 border-t flex-wrap">
                                                                ...
                                                                <>
                                                                    <button 
                                                                        onClick={() => {
                                                                            ...
                                                                            if (confirm(`¿Crear gasto en Caja Chica como "${categoria}"?`)) {
                                                                    ...
                                                                    <button 
                                                                        onClick={() => alert('Próximamente: crear factura recibida')}
                                                            ...
                                                            {mov.tipo === 'entrada' && (
                                                                <button 
                                                                    onClick={() => alert('Próximamente: crear factura emitida')}
        ...
        // Modal para Contabilidad
        function ContaModal({ type, item, ufActual, tickets, keyAccounts, onSave, onClose }) {
            const getDefault = () => {
            ...
            const recalcularBoleta = (brutoUF, porcentaje, ufDia) => {
                const bruto = parseFloat(brutoUF) || 0;
                const pct = parseFloat(porcentaje) || 0;
                const uf = ufDia || ufActual;
                const retencionUF = (bruto * pct / 100).toFixed(2);
                const liquidoUF = (bruto - retencionUF).toFixed(2);
                return {
                    monto_retencion_uf: retencionUF,
                    monto_retencion_clp: Math.round(retencionUF * uf),
                    monto_liquido_uf: liquidoUF,
                    monto_liquido_clp: Math.round(liquidoUF * uf)
                };
            };

            // Auto-calcular retención y líquido para boletas
            const handleBoletaMontoChange = (field, value) => {
                const num = parseFloat(value) || 0;
                ...
                if (field === 'monto_bruto_uf') {
                    setForm({
                        ...form,
                        monto_bruto_uf: value,
                        monto_bruto_clp: Math.round(num * (form.uf_dia || ufActual)),
                        ...recalcularBoleta(value, porcentaje, form.uf_dia)
                    });
                } else if (field === 'monto_bruto_clp') {
                    const brutoUF = (num / (form.uf_dia || ufActual)).toFixed(2);
                    setForm({
                        ...form,
                        monto_bruto_clp: value,
                        monto_bruto_uf: brutoUF,
                        ...recalcularBoleta(brutoUF, porcentaje, form.uf_dia)
                    });
                }
            };

            // Cuando cambia el porcentaje, recalcular con el bruto que ya hay
            const handlePorcentajeChange = (value) => {
                setForm({
                    ...form,
                    porcentaje_retencion: value,
                    ...recalcularBoleta(form.monto_bruto_uf, value, form.uf_dia)
                });
            };
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); onSave(form); }} className="space-y-4">
                            {type === 'emitida' && (<>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <InputField label="Cliente" required value={form.cliente} onChange={(e) => setForm({...form, cliente: e.target.value})} />
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <InputField label="Fecha Emisión" type="date" required value={form.fecha_emision} onChange={(e) => setForm({...form, fecha_emision: e.target.value})} />
                                    <SelectField label="Estado" value={form.estado} onChange={(e) => setForm({...form, estado: e.target.value})} options={['Pendiente', 'Pagada', 'Vencida']} />
                                </div>
                                ...
                                <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => setMonedaPrincipal('CLP')}
                                            className={`flex-1 px-4 py-2 rounded-lg text-sm font-medium transition ${
                                                monedaPrincipal === 'CLP'
                                                    ? 'bg-verde text-white'
                                                    : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50'
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    {monedaPrincipal === 'UF' ? (
                                        <>
                                            <InputField label="Monto UF" type="number" step="0.01" required value={form.monto_uf} onChange={(e) => handleMontoChange('monto_uf', e.target.value)} />
                                            <InputField label="Equiv. CLP" type="number" disabled value={form.monto_clp} className="bg-gray-50" />
                                            <InputField label="UF del día" type="number" value={form.uf_dia} onChange={(e) => { setForm({...form, uf_dia: parseFloat(e.target.value)}); handleMontoChange('monto_uf', form.monto_uf); }} />
                                        </>
                                    ) : (
                                        <>
                                            <InputField label="Monto CLP" type="number" required value={form.monto_clp} onChange={(e) => handleMontoChange('monto_clp', e.target.value)} />
                                            <InputField label="Equiv. UF" type="number" step="0.01" disabled value={form.monto_uf} className="bg-gray-50" />
                                            <InputField label="UF del día" type="number" value={form.uf_dia} onChange={(e) => { setForm({...form, uf_dia: parseFloat(e.target.value)}); handleMontoChange('monto_clp', form.monto_clp); }} />
                                        </>
                                    )}
                                </div>
                                <TextAreaField label="Descripción" value={form.descripcion || ''} onChange={(e) => setForm({...form, descripcion: e.target.value})} />
                                {form.estado === 'Pagada' && (
                                    <InputField label="Fecha Pago" type="date" value={form.fecha_pago || ''} onChange={(e) => setForm({...form, fecha_pago: e.target.value})} />
                                )}
                            </>)}
                            
                            {type === 'recibida' && (<>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <InputField label="Proveedor" required value={form.proveedor} onChange={(e) => setForm({...form, proveedor: e.target.value})} />
                                    <SelectField label="Categoría" required value={form.categoria} onChange={(e) => setForm({...form, categoria: e.target.value})} options={['Honorarios', 'Servicios', 'Oficina', 'Marketing', 'Tecnología', 'Sueldos', 'Otros']} />
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <InputField label="Fecha Emisión" type="date" required value={form.fecha_emision} onChange={(e) => setForm({...form, fecha_emision: e.target.value})} />
                                    <SelectField label="Estado" value={form.estado} onChange={(e) => setForm({...form, estado: e.target.value})} options={['Pendiente', 'Pagada']} />
                                </div>
                            ...
                            {type === 'sueldo' && (<>
                                ...
                                <div className="flex items-center gap-4 p-3 bg-gray-50 rounded">
                                    ...
                                    <button 
                                        type="button"
                                        onClick={() => setMonedaPrincipal('UF')} 
                                        className={`px-3 py-1 rounded text-sm ${monedaPrincipal === 'UF' ? 'bg-verde text-white' : 'bg-gray-200'}`}
                                    >
                                        UF
                                    </button>
                                    <button 
                                        type="button"
                                        onClick={() => setMonedaPrincipal('CLP')} 
                                        className={`px-3 py-1 rounded text-sm ${monedaPrincipal === 'CLP' ? 'bg-verde text-white' : 'bg-gray-200'}`}
                                    >
                                        CLP
                                    </button>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <input 
                                            type="number" 
                                            step="0.01"
                                            value={monedaPrincipal === 'UF' ? (form.monto_uf || '') : (form.monto_clp || '')} 
                                            onChange={(e) => handleMontoChange(monedaPrincipal === 'UF' ? 'monto_uf' : 'monto_clp', e.target.value)}
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            type="number" 
                                            step="0.01"
                                            value={monedaPrincipal === 'UF' ? (form.monto_clp || '') : (form.monto_uf || '')}
                                            disabled
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            value={form.uf_dia} 
                                            onChange={(e) => {
                                                const nuevoUF = parseFloat(e.target.value) || ufActual;
                                                setForm({...form, uf_dia: nuevoUF});
                                            }}
                                            className="w-full border rounded p-2" 
                                        />
                                    </div>
                            ...
                            {type === 'caja' && (<>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <InputField label="Fecha" type="date" required value={form.fecha} onChange={(e) => setForm({...form, fecha: e.target.value})} />
                                    <InputField label="Monto CLP" type="number" required value={form.monto_clp} onChange={(e) => setForm({...form, monto_clp: e.target.value})} />
                                </div>
                                <InputField label="Concepto" required value={form.concepto} onChange={(e) => setForm({...form, concepto: e.target.value})} />
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <SelectField label="Categoría" value={form.categoria} onChange={(e) => setForm({...form, categoria: e.target.value})} options={['Transporte', 'Alimentación', 'Materiales', 'Otros']} />
                                    <InputField label="Responsable" value={form.responsable || ''} onChange={(e) => setForm({...form, responsable: e.target.value})} />
                                </div>
                                <InputField label="Comprobante (opcional)" value={form.comprobante || ''} onChange={(e) => setForm({...form, comprobante: e.target.value})} />
                            </>)}
                            
                            {type === 'boleta' && (<>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <InputField label="Prestador" required value={form.prestador} onChange={(e) => setForm({...form, prestador: e.target.value})} placeholder="Seba, Max, Patxi..." />
                                    <InputField label="Mes Servicio" required value={form.mes_servicio} onChange={(e) => setForm({...form, mes_servicio: e.target.value})} placeholder="Enero 2026" />
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <InputField label="Fecha" type="date" required value={form.fecha} onChange={(e) => setForm({...form, fecha: e.target.value})} />
                                    <InputField label="RUT (opcional)" value={form.rut || ''} onChange={(e) => setForm({...form, rut: e.target.value})} placeholder="12.345.678-9" />
                                </div>
                                ...
                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        {monedaPrincipal === 'UF' ? (
                                            <>
                                                <InputField label="Monto Bruto UF" type="number" step="0.01" required value={form.monto_bruto_uf} onChange={(e) => handleBoletaMontoChange('monto_bruto_uf', e.target.value)} />
                                                <InputField label="Equiv. CLP" type="number" disabled value={form.monto_bruto_clp} className="bg-gray-50" />
                                                <InputField label="UF del día" type="number" value={form.uf_dia} onChange={(e) => setForm({...form, uf_dia: parseFloat(e.target.value)})} />
                                            </>
                                        ) : (
                                            <>
                                                <InputField label="Monto Bruto CLP" type="number" required value={form.monto_bruto_clp} onChange={(e) => handleBoletaMontoChange('monto_bruto_clp', e.target.value)} />
                                        ...
                                        <div className="flex gap-2 flex-wrap">
                                            {[
                                                ...
                                                <button
                                                    type="button"
                                                    key={opt.value}
                                                    onClick={() => handlePorcentajeChange(opt.value)}
                                                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                                                        parseFloat(form.porcentaje_retencion) === parseFloat(opt.value)
                                                            ? 'bg-blue-600 text-white'
                                            ...
                                            <input
                                                ...
                                                className="w-20 px-2 py-1.5 border border-blue-200 rounded-lg text-xs text-center"
                                                placeholder="%"
                                            />
                                        </div>
                                    </div>
                                ...
                                <TextAreaField label="Descripción del servicio" value={form.descripcion || ''} onChange={(e) => setForm({...form, descripcion: e.target.value})} />
    ...
    // Exportar como variable global para carga dinámica
    window.ContabilidadModule = ContabilidadView;
