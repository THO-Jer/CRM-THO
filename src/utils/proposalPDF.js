...
// Service catalog with default descriptions and deliverables
...
export function generateProposal({ organizacion, contacto, tipo, valor, moneda = 'UF', ufActual, notas, fecha }) {
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
    const W = 210, H = 297;
    const ML = 25, MR = 25; // margins
    const CW = W - ML - MR; // content width
    let y = 0;

    const service = SERVICE_CATALOG[tipo] || {
        nombre: tipo,
    ...
    doc.text(service.nombre, ML, y);

    // Divider line
    y += 12;
    setDraw(COLORS.naranja);
    ...
    // ========== PAGE 2: DESCRIPTION + DELIVERABLES ==========
    doc.addPage();
    y = 30;
    ...
    doc.setFontSize(14);
    setColor(COLORS.naranja);
    doc.text('1. Descripción del Servicio', ML, y);
    y += 3;
    setDraw(COLORS.naranja);
    ...
    // Section 2: Entregables
    y += 10;
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(14);
    setColor(COLORS.naranja);
    doc.text('2. Entregables', ML, y);
    y += 3;
    doc.line(ML, y, ML + 50, y);
    y += 10;

    service.entregables.forEach((ent, i) => {
        // Numbered item with subtle background
    ...
    doc.setFontSize(14);
    setColor(COLORS.naranja);
    doc.text('3. Plazo de Ejecución', ML, y);
    y += 3;
    doc.line(ML, y, ML + 50, y);
    y += 10;
    ...
    if (isRecurrente) {
        doc.text('Facturación mensual. Valores en UF, no sujetos a variación CLP.', ML + 10, y + 31);
    } else {
        doc.text('Valores en UF, no sujetos a variación CLP. Facturación al inicio del servicio.', ML + 10, y + 31);
    }
    ...
    doc.setFontSize(14);
    setColor(COLORS.naranja);
    doc.text('5. Condiciones Generales', ML, y);
    y += 3;
    doc.line(ML, y, ML + 50, y);
    y += 10;
    ...
    doc.setFontSize(10);
    setColor(COLORS.dark);
    doc.text('The Human Org SpA', ML, y); y += 6;
    setColor(COLORS.gray);
    doc.text('contacto@tho.cl  ·  tho.cl', ML, y); y += 6;
    doc.text('Concepción, Chile', ML, y);
