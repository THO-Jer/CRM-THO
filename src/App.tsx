                ...
                <div className="bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700 py-3">
                    <div className="max-w-7xl mx-auto px-4">
                        ...
                        {globalSearch.length >= 2 && (() => {
                            ...
                            const typeIcon: Record<string, string> = { prospecto: '🎯', ticket: '🎫', keyaccount: '🔑', cerrado: '📜' };
                            return results.length > 0 ? (
                                <div className="mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg border dark:border-gray-700 overflow-hidden">
                                    {results.map((r, i) => (
                                        <button key={`${r.type}-${(r.item?.id as string) ?? i}`} onClick={() => { setActiveTab(r.tab); setGlobalSearch(''); setShowGlobalSearch(false); }} className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-gray-700 text-left border-b dark:border-gray-700 last:border-0 transition">
            ...
            <div className="max-w-7xl mx-auto px-4 pt-3">
                <div className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1.5">
                    ...
                    <span className="text-gray-600 dark:text-gray-300 font-medium">
                        {({ dashboard: 'Dashboard', pipeline: 'Pipeline', tickets: 'Tickets', keyaccounts: 'Key Accounts', cerrados: 'Historial', reportes: 'Reportes', 'finanzas-dashboard': 'Dashboard Financiero', contabilidad: 'Estado de Resultados', conciliacion: 'Conciliación Bancaria' } as Record<string, string>)[activeTab]}
                    </span>
                </div>
